...
public class Character1 : MonoBehaviour {
	...
	void BFS(Vector2 position, int index){
		...
		if (Map.tiles [(int)position.y, (int)position.x] == 2) {
			...
			while (true) {
				//percorro a lista de verticies coloridos(visitados)
				for (int i = 0; i < colorVectice.Count; i++) {
		...
		if (traject!=null && traject.Count > 0) {
			...
			if (traject [0] == currentPosition) {
				...
				if (traject.Count==1) {
					...
					Map.tiles [(int)traject [0].y, (int)traject [0].x] = 1;
					Map.SetTile (  ((int)traject [0].y)+"-"+((int)traject [0].x) );
				}
